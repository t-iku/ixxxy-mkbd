...
- **対応スイッチ**: Cherry MX互換キースイッチ (ホットスワップ非対応)
- **イメージ**: TinyGoなどでファームウェアを自作可能。 自分でビルドしてキー割り当てを自由に変更してください。

### 基板の向きについて

**注意**: シルク印刷でイラストが描いてある面が裏面（はんだ付けする面）です。

![基板両面（表面／裏面）](./DSC_1032.jpeg)
...
|品名               |数量|備考     |
|-----------------|--|-------|
|Seeed XIAO RP2040|1 |[マイコンボード](https://akizukidenshi.com/catalog/g/g117044/)|
...
1. ゴム足を基板裏面の四隅に貼り付けます
   - 基板が安定する位置に貼ってください
   - xiao rp2040のピンヘッダのはんだ付けした位置と干渉して貼りづらい可能性があります。すみません。
...
1. ファームウェアのビルドと書き込み
   ...
   ```bash
   # BOOTSELボタン (Bと書かれたボタン) を押しながらUSBケーブルを接続
   # ストレージデバイスとして認識されます
   
   # プロジェクトディレクトリで実行
   tinygo flash -target xiao-rp2040 main.go

   # またはファイルエクスプローラーを開いてイメージファイル (.uf2ファイル) をコピー
   ```
